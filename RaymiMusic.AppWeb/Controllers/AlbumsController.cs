    ...
    public class AlbumsController : Controller
    {
        ...
        public IActionResult Create()
        {
            ...
            if (User.IsInRole("admin"))
            {
                ...
                // Depuración: Imprime los nombres de los artistas cargados
                var artistas = _ctx.Artistas.ToList();
                Console.WriteLine($"Artistas: {string.Join(", ", artistas.Select(a => a.NombreArtistico))}");
            }
            else
            {
        ...
        public async Task<IActionResult> Create(AlbumCreateVM vm)
        {
            ...
            var entidad = new Album
            {
                Id = Guid.NewGuid(),
                Titulo = vm.Titulo,
                FechaLanzamiento = vm.FechaLanzamiento?.ToUniversalTime() ?? DateTime.UtcNow,
        ...
        public async Task<IActionResult> Edit(Guid id, AlbumEditVM vm)
        {
            ...
            album.FechaLanzamiento = vm.FechaLanzamiento?.ToUniversalTime() ?? DateTime.UtcNow;
